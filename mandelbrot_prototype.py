...
def smooth_iter(c, max_iter):
    ...
    z = 0j  # Initialize z to zero in the complex plane.
...
@jit
def color_pixel(matrix, n_iter, colortable, n_cycle):
    ...
    for i in range(3):
        matrix[i] = colortable[col_i, i]
        matrix[i] = max(0, min(1, matrix[i]))


# Define a function to compute the Mandelbrot set and color it.
@jit(parallel=True)
def compute_set(real_part, imaginary_part, max_iter, colortable, n_cycle):
    ...
    for x in prange(x_pixels):
        for y in range(y_pixels):
            ...
            # Calculate the iteration count and other parameters for this point.
            niter = smooth_iter(c, max_iter)
            # Color the pixel if the iteration count is greater than 0.
            if niter > 0:
                color_pixel(mat[y, x], niter, colortable, n_cycle)
    return mat  # Return the colored image matrix.


# Define a function to compute the Mandelbrot set using GPU acceleration.
@cuda.jit
def compute_set_gpu(matrix, x_min, x_max, y_min, y_max, max_iter, colortable, n_cycle):
    ...
    if (y < matrix.shape[0]) and (x < matrix.shape[1]):
        ...
        # Color the pixel if the iteration count is greater than 0.
        if niter > 0:
            color_pixel(matrix[y, x], niter, colortable, n_cycle)
...
class Mandelbrot:
    ...
    def update_set(self):
        ...
        if self.gpu:
            ...
            compute_set_gpu[n_block, n_thread](self.set, *self.coord, self.max_iterations, self.colortable, n_cycle)
        else:
